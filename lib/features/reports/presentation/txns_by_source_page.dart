...
class TxnsBySourcePage extends StatefulWidget {
  final List<AirtelMoneyTxn> txns;
  const TxnsBySourcePage({super.key, required this.txns});

  @override
  State<TxnsBySourcePage> createState() => _TxnsBySourcePageState();
}

enum PeriodPreset { today, week, month, custom }
enum NetworkFilter { all, airtel, mtn }

class _TxnsBySourcePageState extends State<TxnsBySourcePage> {
  ...
  String _sourceLabel(AirtelMoneyTxn t) {
  ...
  // ---------- compute ----------
  List<AirtelMoneyTxn> _filteredCredits() {
    final range = _activeRange();
    final start = range.start;
    final end = range.end;

    final list = widget.txns.where((t) {
      if (t.type != AirtelTxnType.credit) return false; // ✅ credits only
  ...
  List<_SourceGroup> _groupBySource(List<AirtelMoneyTxn> credits) {
    final map = <String, _SourceGroup>{};
    ...
    for (final t in credits) {
      ...
      final label = _sourceLabel(t);

      final existing = map[key];
      if (existing == null) {
        map[key] = _SourceGroup(
          key: key,
          label: label,
          total: t.amount,
          count: 1,
  ...
  Widget build(BuildContext context) {
    ...
    final groups = _groupBySource(credits);

    final range = _activeRange();
    final rangeText = "${_fmtDate(range.start)} → ${_fmtDate(range.end)}";

    final totalIn = credits.fold<int>(0, (sum, t) => sum + t.amount);

    return Scaffold(
      appBar: AppBar(
        title: const Text("Credits by Source"),
      ),
      body: Padding(
                    ...
                    children: [
                      ...
                      DropdownButton<NetworkFilter>(
                        ...
                        items: const [
                          ...
                          DropdownMenuItem(
                            value: NetworkFilter.airtel,
                            child: Text("Airtel"),
                          ),
                          DropdownMenuItem(
                            value: NetworkFilter.mtn,
                            child: Text("MTN"),
                          ),
                        ],
                      ),
...
class _SourceDetailsPage extends StatelessWidget {
  final String title;
  final List<AirtelMoneyTxn> txns;

  const _SourceDetailsPage({required this.title, required this.txns});
